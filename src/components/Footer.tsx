...
export function Footer() {
  return (
    <footer className="bg-slate-900/50 backdrop-blur-lg">
      <div className="mx-auto max-w-7xl overflow-hidden px-6 py-12 sm:py-16 lg:px-8">
        ...
        >
          {navigation.map((item) => (
            <div key={item.name} className="pb-6">
              <Link
                href={item.href}
                className="text-sm leading-6 text-slate-400 hover:text-primary-500 transition-colors"
              >
                {item.name}
              </Link>
            </div>
        ...
        <div className="mt-10 flex justify-center space-x-10">
          ...
          <a
            href="https://github.com/aryxn1233"
            className="text-slate-500 hover:text-slate-400 transition-colors"
          >
        ...
        <p className="mt-10 text-center text-xs leading-5 text-slate-500">
          &copy; {new Date().getFullYear()} BPI, Inc. All rights reserved.
        </p>
      </div>
    </footer>
  );
